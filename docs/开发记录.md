...
1. **新增 Rust 命令 `batch_split_prompt`**
...
2. **新增类型定义**
   - `PromptSegmentItem` - 拆分后的片段项
   - `BatchSplitResult` - 批量拆分结果
   - 前端类型：`src/types/index.ts`

3. **注册 Tauri 命令**
   - 在 `src-tauri/src/lib.rs` 中注册 `batch_split_prompt`
...
5. **API调用**
   - `src/api.ts` 添加 `batchSplitPrompt` 函数
...
7. **需求文档更新**
   - 添加US-008~US-012五个User Story
   - 为每个Story拆分成详细的Task（共34个Task）
...
- [x] 前端UI：分隔符配置和拆分结果显示
- [x] 前后端联调测试
- [x] US-009: 批量角色绑定及并发生成功能

---

## US-009: 批量角色绑定及并发生成功能
...
1. **并发数配置功能**
   - 后端 `GenerationConfig` 添加 `concurrency` 字段
   - 前端添加并发数状态管理
   - 在生成参数区域添加并发数输入框 (1-5)
   - 配置可保存和加载

2. **批量拆分结果角色绑定**
   - 在批量拆分结果展开面板中添加角色绑定UI
   - 每个场景可单独绑定角色参考图
   - 显示绑定状态（已绑定/未绑定）
   - 支持绑定和解绑操作
...
3. **批量生成设置弹框**
   - 在"全部生图"按钮旁添加"设置"按钮
   - 弹框包含：模型选择、并发数
   - Seedream 参数：尺寸、质量、组图、格式、水印
   - Banana 2 参数：比例、分辨率

4. **模型名称统一**
   - "Banana Pro" 改为 "Banana 2"

5. **开发文档**
   - `docs/US-009_开发记录_20260301_1600.md` - 开发记录
   - `docs/US-009_测试案例_20260301_1600.md` - 测试案例
